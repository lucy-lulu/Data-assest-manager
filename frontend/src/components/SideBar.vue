...
export default {
  ...
  data() {
    return {
      icons: [
        ...
        { src: 'collect.png', action: 'route', link: '/assetCollection' },
        { src: 'add.png', action: 'route', link: '/assetAddition' },
  ...
  computed: {
    ...
    filteredIcons() {
      if (this.role === 3) {
        return this.icons.filter(icon => icon.action === 'logout' || icon.src === 'settings.png' || icon.src === 'account.png' || icon.src === 'home.png');
